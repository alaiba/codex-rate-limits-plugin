...
This repository is the dedicated home for the `codex-rate-limits` plugin. Multiple closely related skills can live inside the same plugin package when they share the same owner, release cadence, support boundary, and validation workflow. Unrelated plugins should usually live in separate repositories.
...
2. Copy or sync `plugins/codex-rate-limits/` to `~/plugins/codex-rate-limits/`.
...
Keep the installable content under `plugins/codex-rate-limits/` free of repository-specific assumptions. The standalone plugin package should be portable as-is into other repositories, shared plugin mirrors, or home-local plugin directories without code changes.
